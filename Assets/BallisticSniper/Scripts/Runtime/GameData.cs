    ...
    public static class GameRules
    {
        ...
        public static readonly OperationDefinition[] OperationDefinitions =
        {
            new OperationDefinition(
                "ТИХАЯ ВСТРЕЧА",
                "Городская встреча • наблюдение за группой людей",
                "Бордовый пиджак • светлая рубашка • перемещается по площадке",
                "Несколько людей движутся рядом и периодически перекрывают обзор. Идентифицируйте нужного персонажа.",
                280,
                4,
                OperationKind.Conversation),
            new OperationDefinition(
                "ОКНО ОТЕЛЯ",
                "Наблюдение за номером среди городской застройки",
                "Тёмно-зелёная куртка • появляется у окна",
                "Рама закрывает часть силуэта, персонаж отходит от окна, другие NPC проходят перед ним.",
                420,
                4,
