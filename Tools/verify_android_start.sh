...
for check_index in $(seq 1 120); do
  ...
  if grep -Fq "BALLISTIC_ANDROID_MENU_READY version=5.5.0 screen=Menu" "$RESULTS_DIR/android-logcat.txt"; then
    menu_ready=1
    break
  fi
...
for check_index in $(seq 1 60); do
  ...
  if grep -Fq "BALLISTIC_ANDROID_MISSION_BRIEFING version=5.5.0 stage=1" "$RESULTS_DIR/android-mission-logcat.txt"; then
    briefing_ready=1
    break
  fi
  sleep 0.20
